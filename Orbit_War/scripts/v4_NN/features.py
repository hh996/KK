...
def encode_state(world: WorldState, perspective_player=None, device=None) -> torch.Tensor:
    ...
    grid = np.zeros((CHANNELS, H, W), dtype=np.float32)
    step_norm = min(1.0, world.step_count / 500.0)
    opp_ids = sorted([oid for oid in world.player_ids if oid != perspective_player])[:3]
                    ...
                    if 0 <= nx < W and 0 <= ny < H:
                        if p.owner == perspective_player:
                            ...
                            if idx < 3:
                                ...
                                grid[base_ch + 1, ny, nx] = max(
                                    grid[base_ch + 1, ny, nx], p.production / 5.0
                                )
    ...
    grid[14, :, :] = step_norm
    grid[15, :, :] = (float(len(world.player_ids)) - 2.0) / 2.0
    grid[16] = _sun_mask
