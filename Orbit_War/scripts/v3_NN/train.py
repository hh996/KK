...
def play_one_game(net):
    """2/4 人混合自博弈；返回 (state, value, src_pi, tgt_pi, ship_pi)。"""
    ...
    def make_agent(player_id):
        def agent(obs, config):
            ...
            mcts_inst = MCTS(net, c_puct=C_PUCT, num_simulations=TRAIN_MCTS_SIMULATIONS)
    ...
    for pid, state_np, src_pi, tgt_pi, ship_pi in game_history:
        target_v = env_terminal_value(scores, pid, participant_ids=participant_ids)
...
def _trainer_eval(net):
    try:
        from eval_orbit_wars import evaluate_net_vs_random

        wins, total = evaluate_net_vs_random(
            net, episodes=min(8, GAME_EVAL_EPISODES), num_agents=2
        )
        print(f"  [eval] vs random ({total} ep 2p): {wins} wins")
    except Exception as ex:
        print(f"  [eval] skipped: {ex}")
...
class Trainer:
    ...
    def load_checkpoint(self, name):
        ...
        self.replay_buffer.buffer = deque(
            checkpoint["replay_buffer"], maxlen=REPLAY_BUFFER_SIZE
        )
        return True
    ...
    def run(self):
        ...
        while self.iteration < MAX_ITERATIONS:
            ...
            for _ in range(NUM_PARALLEL_GAMES):
                samples = play_one_game(self.net)
                for item in samples:
