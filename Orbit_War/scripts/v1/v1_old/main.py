"""Orbit Wars v2 - Unified delta-V value function with 1-step beam-search lookahead.

Strategy core:
  1. Generate <= BEAM_WIDTH candidate action sets per turn via greedy beam.
  2. Evaluate each candidate with a single value function V at HORIZON_VALUE,
     after simulating 1 step and the opponent's simplified best-response.
  3. Pick the action set with the highest V(me) - max V(opponent).
  4. Always evaluate the no-op as a baseline; on timeout fall back to the
     candidate with the highest immediate heuristic value.

Physics core (preserved from v1.2):
  - Real fleet-speed formula: 1 + 5 * (ln(ships) / ln(1000))^1.5
  - Per-planet signed angular velocity reverse-engineered from initial state
  - Iterative interception with damping
  - Path-blockage check using line-circle intersection vs. all other planets
  - Detour retry on +/- {5, 10, 18} degrees when direct path is blocked
"""
...
HORIZON_VALUE = 120          # turns to project for V scoring
HORIZON_SIM = 80             # max turns to trace fleet arrivals
BEAM_WIDTH = 6               # max number of atoms stacked per candidate set
MAX_TARGETS_PER_SRC = 4      # top targets considered per source planet
MAX_SOURCES = 6              # consider strongest N source planets
PROD_DISCOUNT = 0.85         # production discount factor for value
FRONTIER_BONUS = 4.0         # bonus for own planets close to enemies
PLANET_OWN_BONUS = 2.0       # flat per-production bonus for owning a planet
DETOUR_OFFSETS_DEG = (5, -5, 10, -10, 18, -18)
SOFT_DEADLINE = 0.85         # max wall time per turn (seconds)
OPP_TIME_FRACTION = 0.55     # share of remaining budget for opponent simulation
OPP_MAX_EVAL = 5             # max opp candidates to evaluate per opp
...
class Atom:
    """A single proposed launch action."""
...
def fleet_speed(ships, max_speed=MAX_SPEED):
    """Real env formula: 1 + (max-1) * (ln ships / ln 1000)^1.5, capped at max."""
    if ships <= 1:
        return 1.0
...
def estimate_signed_omega(planet, init_pos, step, base_omega):
    """Reverse-engineer the signed angular velocity for one planet
    by comparing its current and initial angle to (+/- base_omega) * step."""
...
def compute_intercept(src, target, ships, omega_map, cid_to_group, comet_ids,
    ...
    """Iterative fixed-point solve for (angle, eta) that hits a moving target."""
    if ships <= 0:
        return None, None
    ...
    for _ in range(max_iter):
        ...
        if abs(d_t) < t_tol and abs(d_a) < ang_tol:
            ...
            break
        # 0.6 damping factor avoids oscillation on fast-rotating targets
...
def path_blocked_by_other_planet(src, target, angle, eta, ships, planets,
    ...
    """Frame-by-frame line-circle check vs every non-target planet (including friendlies)."""
    sx, sy = get_launch_position(src, angle)
    speed = fleet_speed(ships)
...
def trace_intercept(src, angle, target, speed, world, max_turns=HORIZON_SIM):
    """Ray-trace: given a forced angle, find the turn count when the line enters
    the target's predicted disk. Returns float eta or None."""
    sx, sy = get_launch_position(src, angle)
    cos_a, sin_a = math.cos(angle), math.sin(angle)
    prev_x, prev_y = sx, sy
...
def compute_intercept_with_detour(src, target, ships, world):
    """Direct intercept first; on block, try +/- offsets to detour around friendly planets.
    Returns (angle, eta, detour_deg) or None."""
...
def predict_fleet_arrival(fleet, planets, omega_map, cid_to_group, comet_ids,
    ...
    """For an EXISTING fleet in obs, predict (eta, target_planet_id)
    by ray-tracing its straight-line trajectory against all moving planets.
    Returns (max_turns + 1, None) if it leaves the board or hits the sun."""
...
class WorldState:
    def __init__(self, player, step, planets, fleets, initial_planets, base_omega,
        ...
        # Existing-fleet arrivals are immutable across all candidate evaluations
        self.fleet_arrivals = {}
        for f in fleets:
...
def simulate_planet_timeline(planet, arrivals, horizon):
    """Return a list of (owner, ships) at every turn in [0, horizon].

    Combat resolution per README:
      - Group attackers by owner, sum ships.
      - Two largest fight: difference survives. Tie-at-top -> all attackers die.
      - If survivor is current owner: garrison += survivor.
      - If survivor != owner: garrison -= survivor; if garrison < 0, owner flips
        and new garrison = -garrison.
    """
    ...
    for turn in range(1, horizon + 1):
        ...
        if turn in by_turn:
            ...
            if len(sorted_attackers) >= 2 and second_ships == top_ships:
                pass  # tied attackers eliminate each other
            else:
...
def project_state(world, all_actions):
    """Build per-planet timelines given existing fleets + new actions.

    all_actions: iterable of (player_id, target_id, eta, ships).
    target_id may be None (lost fleet); such fleets contribute nothing.
    """
...
def V_at_horizon(timelines, world, player, horizon):
    """V = sum_owned (ships + production_bonus + frontier_bonus)
        + sum_inflight (ships, accounted for via timeline arrivals)."""
    ...
    # In-flight contribution at turn 0 already encoded into arrivals.
    # Add a small extra for ships still in transit (not yet committed to a
    # target, e.g., went off-board): treat as zero contribution.
    return score
...
def top_targets_for_player(src, world, player, top_k):
    """Cheap heuristic ranking of targets per source planet."""
    ...
    for tid, tgt in world.planets.items():
        ...
        if tgt.owner == -1:
            val *= 1.2  # neutrals are slightly easier value
...
def ship_options(src, tgt, max_ships):
    """Three ship-count probes per (src, tgt): just-enough, comfortable, full."""
    if max_ships <= 0:
        return []
...
def make_atom(src, tgt, ships, world):
    ...
    if eta > HORIZON_SIM:
        return None
    # Heuristic immediate value drives beam ordering
    ...
    if tgt.owner != -1 and tgt.owner != src.owner:
        value *= 1.15  # capturing an enemy planet is worth more
    return Atom(src.id, tgt.id, ships, angle, eta, value)


def generate_candidates(world, player_id, max_sets=BEAM_WIDTH):
    """Greedy beam: nested action sets, each adds the best atom from a fresh source."""
    ...
    # Per-source best atom (highest value), keep distinct sources
    best_per_src = {}
    for atom in atoms:
    ...
    # Also include a "second-best of top source" alternative for diversity
...
def best_response(world, opp_id, my_acts, deadline, max_eval=OPP_MAX_EVAL):
    """Opponent's simplified best response: pick action set maximizing their V."""
...
def evaluate(world, my_atoms, deadline):
    """1-step lookahead: simulate my actions + each opponent's best response,
    then return delta_V(me, max_opp)."""
